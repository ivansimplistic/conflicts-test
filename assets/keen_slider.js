...
function KeenSlider(initialContainer, initialOptions = {}) {
  ...
  function slidesRefreshADA() {
    ...
    _slides.forEach(function(slide, key){
      let focusableElements = Array.from(slide.querySelectorAll('a,input:not([type="hidden"]),select,button,textarea,video,iframe,.btn,.focusable')).filter(x=>!x.closest('.ignore-slide-tabindex'));
